...
import { fetchCollection, addDocument, updateDocument, deleteDocument } from '@/lib/firebase-utils';
...
interface PortfolioContextType {
  ...
  updateCertificate: (id: string, certificate: CertificateType) => Promise<CertificateType>;
  ...
  updateExperience: (id: string, experience: ExperienceType) => Promise<ExperienceType>;
  ...
  updateAchievement: (id: string, achievement: AchievementType) => Promise<AchievementType>;
...
export const PortfolioProvider = ({ children }: { children: ReactNode }) => {
  ...
  const updateCertificate = async (id: string, certificate: CertificateType) => {
    try {
      const updatedCertificate = await updateDocument('certificates', id, certificate);
      setCertificates(prev => 
        prev.map(cert => cert.id === id ? { ...cert, ...certificate } : cert)
      );
  ...
  const updateExperience = async (id: string, experience: ExperienceType) => {
    try {
      const updatedExperience = await updateDocument('experiences', id, experience);
      setExperiences(prev => 
  ...
  const updateAchievement = async (id: string, achievement: AchievementType) => {
    try {
      const updatedAchievement = await updateDocument('achievements', id, achievement);
